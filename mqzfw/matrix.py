...
class MatrixScanner:
    def __init__(
        ...
        rowPins = [digitalio.DigitalInOut(y) for y in rows]
        colPins = [digitalio.DigitalInOut(x) for x in cols]
        if self.diode_orientation == DiodeOrientation.COLUMNS:
            self.outputs = colPins
            self.inputs = rowPins
            self.translate_coords = True
        elif self.diode_orientation == DiodeOrientation.ROWS:
            self.outputs = rowPins
            self.inputs = colPins
    ...
    def scan_for_changes(self):
        ...
        if any_changed:
            int_coord = self.len_cols * row + col + self.offset
            return KeyEvent(int_coord, pressed)
